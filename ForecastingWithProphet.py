# -*- coding: utf-8 -*-
"""
Created on Fri Mar 20 16:35:42 2026
...
st.title("⚡ Energy & Weather Intelligence System (Forecasting!)")


st.sidebar.header("🔧 SQL Connection")
server = st.sidebar.text_input("Server Name", value="localhost")
database = st.sidebar.text_input("Database Name", value="DWH")
username = st.sidebar.text_input("User Name", value="sa")
password = st.sidebar.text_input("Password", type="password", value="didelme")
...
if st.button(" Run Analysis & Generate Report"):
    try:
        with st.spinner('Processing... Please wait.'):
            ...
            query = f"""
                SELECT Date_Hour, Amount 
                FROM DWH_2026_02_24 
                WHERE Devicedescription = 'Chiosco parco' 
                AND Input_Output = 'Output' 
                AND Date_Hour BETWEEN '{start_date}' AND '{end_date}'
            """
            ...
            # data modeling and prophet training 
